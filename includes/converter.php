...
define('AMP_THEME_MINIFY_COMMENT_CSS', '/\*[\s\S]*?\*/');
define('AMP_THEME_MINIFY_STRING', '"(?:[^"\\\]|\\\.)*"|\'(?:[^\'\\\]|\\\.)*\'');
define('AMP_THEME_X', "\x1A"); // escape character

if( !class_exists( 'amp_converter' ) ) {
	class amp_converter {
		...
		public static function minify_css($input, $comment = 2, $quote = 2) {
		    ...
		    $output = $prev = "";
		    foreach (self::minify(array(AMP_THEME_MINIFY_COMMENT_CSS, AMP_THEME_MINIFY_STRING), $input) as $part) {
		...
		*/
		public static function callback_css_union($m) {
			return $m[1] . preg_replace('#\s+#', AMP_THEME_X, $m[2]) . ')';
		}
		...
		public static function minify_css_union($input) {
		    ...
		    $input = preg_replace(array(
		        ...
		        AMP_THEME_X . '$1',
		        // [^2]
		        '$1' . AMP_THEME_X, AMP_THEME_X . '(', ')' . AMP_THEME_X,
		        // [^3]
		        '#$1$2$3',
		        // [^4]
		        '$1',
		    ...
		    return trim(str_replace(AMP_THEME_X, ' ', $input));
		}
